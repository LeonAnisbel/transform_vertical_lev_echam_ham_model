...
cdo expr,du_immerse=NDUSOL_STRAT_A+NDUSOL_STRAT_C ${i} tmpsum.nc
cdo selname,rhoam1 ${fnm}_vphysc.nc tmpdens.nc
cdo merge tmpsum.nc tmpdens.nc tmpmix.nc

cdo -selname,du_immerse tmpmix.nc tmpOA.nc
cdo setunit,'m-3' -setcode,157 -selname,du_immerse tmpOA.nc tmp1.nc
cdo merge tmp0.nc tmp1.nc tmp2.nc
#cdo sp2gpl tmp2.nc tmp3.nc

# interpolation to pressure levels from file pres.txt
cdo after tmp2.nc ${fnm2}_du_immerse_plev.nc << EON
	TYPE=30 CODE=157  LEVEL=${plev}
EON
   #TYPE=30 CODE=157  LEVEL=${plev}
# SW line # TYPE=30 CODE=157 UNITSEL=1 FORMAT=2 
# BH line #TYPE=20 MEAN=0 INTERVAL=0 
#cdo -R remapcon,t63grid ${fnm2}_du_immerse_plev.nc ${fnm2}_du_immerse_plev_t63.nc
