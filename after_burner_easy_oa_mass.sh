...
#list=$(ls -d /scratch/b/b381361/${exp}/${exp}*$yr*.01_tracer.nc)
...
cdo expr,OA=OC_AS+OC_KS+OC_KI+POL_AS+PRO_AS+LIP_AS tmpnonzeros.nc tmpsum.nc
cdo selname,rhoam1 ${fnm1}_vphysc.nc tmpdens.nc
cdo merge tmpsum.nc tmpdens.nc tmpmix.nc

cdo -mulc,1.2927 -selname,OA tmpmix.nc tmpOA.nc
cdo setunit,'kg/m3' -setcode,157 -selname,OA tmpOA.nc tmp1.nc
cdo merge tmp0.nc tmp1.nc tmp2.nc

# interpolation to pressure levels from file pres.txt
cdo after tmp2.nc ${fnm}_OA_plev.nc << EON
